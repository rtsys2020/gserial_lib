/*
 * circular_buff.c
 *
 *  Created on: Jan 31, 2021
 *      Author: jupiter
 */


#include <string.h>
#include "circular_buff.h"



int cb_init(circular_buffer *cb, size_t capacity, size_t sz) {
  const int incremented_capacity = capacity + 1; // Add extra element to evaluate count
  cb->buffer = malloc(incremented_capacity * sz);
  if (cb->buffer == NULL)
    return CB_MEMORY_ERROR;
  cb->buffer_end = (char *)cb->buffer + incremented_capacity * sz;
  cb->sz = sz;
  cb->head = cb->buffer;
  cb->tail = cb->buffer;
  return CB_SUCCESS;
}

int cb_free(circular_buffer *cb) {
  free(cb->buffer);
  return CB_SUCCESS;
}

const int _cb_length(circular_buffer *cb) {
  return (char *)cb->buffer_end - (char *)cb->buffer;
}

int cb_availFree(circular_buffer *cb){
    if (cb->head >= cb->tail)
        return (_cb_length(cb) - (cb->head - cb->tail))/cb->sz;
    else
        return (cb->tail - cb->head)/cb->sz - 1;
}

int cb_availData(circular_buffer *cb){
    if (cb->head >= cb->tail)
        return ((cb->head - cb->tail))/cb->sz;
    else
        return (_cb_length(cb)-(cb->tail - cb->head))/cb->sz - 1;
}
int cb_size(circular_buffer *cb){
	return (_cb_length(cb)/cb->sz);
}

int cb_push_back(circular_buffer *cb, const void *item) {
  const int buffer_length = _cb_length(cb);
  const int capacity_length = buffer_length - cb->sz;

  if ((char *)cb->tail - (char *)cb->head == cb->sz ||
      (char *)cb->head - (char *)cb->tail == capacity_length)
    return CB_OVERFLOW_ERROR;

  memcpy(cb->head, item, cb->sz);

  cb->head = (char*)cb->head + cb->sz;
  if(cb->head == cb->buffer_end)
    cb->head = cb->buffer;

  return CB_SUCCESS;
}

int cb_pop_front(circular_buffer *cb, void *item) {
  if (cb->head == cb->tail)
    return CB_EMPTY_ERROR;

  memcpy(item, cb->tail, cb->sz);

  cb->tail = (char*)cb->tail + cb->sz;
  if(cb->tail == cb->buffer_end)
    cb->tail = cb->buffer;

  return CB_SUCCESS;
}

void cb_flush(circular_buffer *cb){
	cb->head = cb->buffer;
	cb->tail = cb->buffer;
}
